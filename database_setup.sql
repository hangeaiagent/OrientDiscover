...
-- 创建相册表索引
CREATE INDEX idx_map_albums_creator ON map_albums(creator_id);
CREATE INDEX idx_map_albums_access_level ON map_albums(access_level);
CREATE INDEX idx_map_albums_created_at ON map_albums(created_at DESC);
...
CREATE INDEX idx_attractions_country_city ON attractions(country, city);
...
-- 创建多语言内容索引
CREATE INDEX idx_attraction_contents_attraction ON attraction_contents(attraction_id);
CREATE INDEX idx_attraction_contents_language ON attraction_contents(language_code);
...
-- 创建媒体资源索引
CREATE INDEX idx_attraction_media_attraction ON attraction_media(attraction_id);
CREATE INDEX idx_attraction_media_type ON attraction_media(media_type);
CREATE INDEX idx_attraction_media_order ON attraction_media(attraction_id, order_index);
...
-- 创建关联表索引
CREATE INDEX idx_album_attractions_album ON album_attractions(album_id);
CREATE INDEX idx_album_attractions_attraction ON album_attractions(attraction_id);
CREATE INDEX idx_album_attractions_order ON album_attractions(album_id, order_index);
...
CREATE INDEX idx_attraction_embeddings_attraction ON attraction_embeddings(attraction_id);
...
CREATE INDEX idx_user_behaviors_album ON user_behaviors(album_id);
...
-- 相册访问策略
CREATE POLICY "users_can_view_public_albums" ON map_albums -- 用户可以查看公开相册
    FOR SELECT USING (access_level = 'public');

CREATE POLICY "users_can_manage_own_albums" ON map_albums -- 用户可以管理自己的相册
    FOR ALL USING (auth.uid() = creator_id);

-- 相册景点关联策略
CREATE POLICY "can_view_public_album_attractions" ON album_attractions -- 可以查看公开相册的景点
    FOR SELECT USING (
        EXISTS (
...
-- 用户行为策略
CREATE POLICY "users_can_manage_own_behaviors" ON user_behaviors -- 用户可以管理自己的行为记录
...
-- 为需要的表创建更新时间戳触发器
CREATE TRIGGER update_map_albums_updated_at BEFORE UPDATE ON map_albums
...
FROM map_albums ma
LEFT JOIN album_attractions aa ON ma.id = aa.album_id
LEFT JOIN (
    SELECT 
        album_id,
...
FROM attractions a
LEFT JOIN attraction_contents ac ON a.id = ac.attraction_id
LEFT JOIN attraction_media am ON a.id = am.attraction_id
...
COMMENT ON TABLE map_albums IS '地图相册表 - 存储用户创建的景点相册信息';
COMMENT ON TABLE attractions IS '景点主表 - 存储景点的基本信息和地理位置';
COMMENT ON TABLE attraction_contents IS '景点多语言内容表 - 存储景点的多语言描述和介绍';
COMMENT ON TABLE attraction_media IS '景点媒体资源表 - 存储景点的图片、视频等媒体资源';
COMMENT ON TABLE album_attractions IS '相册景点关联表 - 定义相册与景点的关联关系';
COMMENT ON TABLE attraction_embeddings IS '向量存储表 - 存储景点内容的向量嵌入，用于AI搜索';
COMMENT ON TABLE user_behaviors IS '用户行为表 - 记录用户对相册的各种操作行为';